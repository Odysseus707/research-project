...
def solve_ilp(nodes, global_features, tree, greed_param=0.5):
    ...
    y = pulp.LpVariable.dicts('y', E, cat='Binary')  # 1 if edge e is selected (part of path to root)

    # Objective: max sum_v g*|Sv|*xv - (1-g)*sum_e ce*ye
    prob += (
        pulp.lpSum([greed_param * len(Sv[v]) * x[v] for v in V])
        - pulp.lpSum([(1-greed_param) * ce[e] * y[e] for e in E])
    )
