...
class networkENV:
    def __init__(self, num_nodes=7, branching_factor=2, global_features=None, global_timestamps=None):
        self.num_nodes = num_nodes
        self.branching_factor = branching_factor
        self.global_features = global_features or ["precipitation", "temp_max", "temp_min", "wind"]
    ...
    def _generate_tree(self):
        ...
        self.logger.info(f"Tree generated with branching_factor={self.branching_factor}, height={self._get_tree_height()}, total_nodes={self.tree.number_of_nodes()}")

    def _get_tree_height(self):
        # Calculate minimum height needed for at least num_nodes
        h = 0
        while (self.branching_factor ** (h + 1) - 1) // (self.branching_factor - 1) < self.num_nodes:
            h += 1
        return h

    def _setup_nodes_and_orchestrator(self):
        # Assign Node objects to all nodes except root (0)
        self.nodes = [Node(node_id=i, features=[], timestamps=[]) for i in range(1, self.tree.number_of_nodes())]
        # Orchestrator is at root (0), pass the tree for BFS/utility logic
        self.orchestrator = Orchestrator(nodes=self.nodes, global_features=self.global_features, tree=self.tree)
